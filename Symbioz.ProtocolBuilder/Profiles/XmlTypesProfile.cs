...
using Symbioz.ProtocolBuilder.XmlPatterns;
using Symbioz.Core;

namespace Symbioz.ProtocolBuilder.Profiles
{
    public class XmlTypesProfile : ParsingProfile
    {
        ...
        public XmlType SearchXmlPattern(string classname)
        {
            string[] results = Directory.GetFiles(Path.Combine(BuilderConfiguration.Instance.Output, OutPutPath), classname + ".xml", SearchOption.AllDirectories);
        ...
        public override void ExecuteProfile(Parser parser)
        {
            ...
            string xmlfile = Path.Combine(BuilderConfiguration.Instance.Output, OutPutPath, relativePath, Path.GetFileNameWithoutExtension(parser.Filename)) + ".xml";
            ...
            Logger.Write(string.Format("Wrote {0}", Path.GetFileNameWithoutExtension(xmlfile) + ".xml"), ConsoleColor.Gray);
        }
    }
}