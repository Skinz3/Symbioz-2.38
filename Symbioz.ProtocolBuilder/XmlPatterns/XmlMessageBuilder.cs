...
namespace Symbioz.ProtocolBuilder.XmlPatterns
{
    public class XmlMessageBuilder : XmlPatternBuilder
    {
        ...
        public override void WriteToXml(XmlWriter writer)
        {
            ...
            string type = null;
            int limit = 0;
            if (Path.GetFileNameWithoutExtension(Parser.Filename) == "IdentificationMessage")
            {
            ...
            for (int i = 0; i < deserializeAsMethod.Statements.Count; i++)
            {
                    ...
                    ((AssignationStatement)deserializeAsMethod.Statements[i]).Value.Contains("getFlag"))
                {
                    ...
                    //  if (match.Success)
                    {
                        var data = statement.Value.Split('(').Last().Split(',').Last();
                        type = "flag(" + data.Replace(")", string.Empty) + ")";
